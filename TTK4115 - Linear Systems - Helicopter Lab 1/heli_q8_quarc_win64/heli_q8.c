 ...
 * C source code generated on : Tue Sep  6 19:35:27 2022
 *
...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
   ...
   *  Sum: '<S6>/Sum'
   */
  heli_q8_B.Sum_m = ((heli_q8_X.Integrator_CSTATE + heli_q8_B.K_ep) -
  ...
  /* Gain: '<Root>/Gain' */
  heli_q8_B.Gain = heli_q8_P.Gain_Gain * rtb_Backgain;
  if (rtmIsMajorTimeStep(heli_q8_M)) {
   ...
   *  Sum: '<S5>/Sum'
   */
  rtb_Backgain = heli_q8_B.Gain - heli_q8_B.PitchCounttorad;

  /* Sum: '<Root>/Sum1' */
  heli_q8_B.Sum1 = rtb_Backgain;
  if (rtmIsMajorTimeStep(heli_q8_M)) {
      ...
      if (rtmIsMajorTimeStep(heli_q8_M) ) {
        {
          if (!(++heli_q8_DW.ToFile_IWORK.Decimation % 1) &&
              (heli_q8_DW.ToFile_IWORK.Count * (7 + 1)) + 1 < 100000000 ) {
            ...
            if (fp != (NULL)) {
              ...
              if (fwrite(u, sizeof(real_T), 7 + 1, fp) != 7 + 1) {
                rtmSetErrorStatus(heli_q8_M,
                                  "Error writing to MAT-file Lab1_case1_7_2.mat");
                return;
              }

              if (((++heli_q8_DW.ToFile_IWORK.Count) * (7 + 1))+1 >= 100000000)
              {
                (void)fprintf(stdout,
                              "*** The ToFile block will stop logging data before\n"
                              "    the simulation has ended, because it has reached\n"
                              "    the maximum number of elements (100000000)\n"
                              "    allowed in MAT-file Lab1_case1_7_2.mat.\n");
              }
            }
          }
        }
   ...
   *  Gain: '<S5>/Gain1'
   */
  rtb_Backgain = rtb_Backgain * heli_q8_P.K_pp - heli_q8_P.K_pd *
   ...
   *  Sum: '<S1>/Add'
   */
  u0 = (heli_q8_B.Sum_m - rtb_Backgain) * heli_q8_P.Frontgain_Gain;
   ...
   *  Sum: '<S1>/Subtract'
   */
  u0 = (heli_q8_B.Sum_m + rtb_Backgain) * heli_q8_P.Backgain_Gain;
...
void heli_q8_initialize(void)
{
  ...
  {
    FILE *fp = (NULL);
    char fileName[509] = "Lab1_case1_7_2.mat";
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(heli_q8_M, "Error creating .mat file Lab1_case1_7_2.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp, 7 + 1, 0, "data")) {
      rtmSetErrorStatus(heli_q8_M,
                        "Error writing mat file header to file Lab1_case1_7_2.mat");
      return;
    }
...
void heli_q8_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "Lab1_case1_7_2.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(heli_q8_M, "Error closing MAT-file Lab1_case1_7_2.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(heli_q8_M,
                          "Error reopening MAT-file Lab1_case1_7_2.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 7 + 1, heli_q8_DW.ToFile_IWORK.Count,
           "data")) {
        rtmSetErrorStatus(heli_q8_M,
                          "Error writing header for data to MAT-file Lab1_case1_7_2.mat");
      }
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (3740554495U);
  heli_q8_M->Sizes.checksums[1] = (3652070254U);
  heli_q8_M->Sizes.checksums[2] = (1561490530U);
  heli_q8_M->Sizes.checksums[3] = (2262390251U);
